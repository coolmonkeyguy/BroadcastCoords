package net.pl3x.broadcastcoords.commands;

import net.pl3x.broadcastcoords.BroadcastCoords;
...
public class CmdCoords implements CommandExecutor {
	...
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		...
		if (args.length > 0) {
			...
			if (player == null) {
				cs.sendMessage(plugin.colorize("&4Player does not exist!"));
				return true;
			}
		} else {
			cs.sendMessage(plugin.colorize("&4You must specify a player!"));
			return false;
		}
		
		String msg = plugin.getConfig().getString("broadcast-format", "&d{name} &7is at&f: &4{x}&7, &4{z}");
